...
var createPreprocessor = function (config, basePath, ppOrder, injector) {
  ...
  var instantiatePreprocessors = function (names, order) {
    var preprocessors = []
    if (order) {
  ...
  patterns.forEach(function (pattern) {
    allPreprocessors = allPreprocessors.concat(config[pattern])
  })
  instantiatePreprocessors(allPreprocessors, ppOrder)
        ...
        for (var i = 0; i < patterns.length; i++) {
          if (mm(file.originalPath, patterns[i], {dot: true})) {
            if (thisFileIsBinary) {
              ...
              preprocessors = preprocessors.concat(config[patterns[i]])
            }
          }
        }

        createNextProcessor(instantiatePreprocessors(preprocessors, ppOrder),
            file, done)(null, thisFileIsBinary ? buffer : buffer.toString())
      })
    })
  }
}
createPreprocessor.$inject = ['config.preprocessors', 'config.basePath',
    'config.preprocessorOrder', 'injector']
